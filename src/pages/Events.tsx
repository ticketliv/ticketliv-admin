...
import { Filter, Plus, Edit3, RefreshCw, Search, Trash2, Calendar as CalIcon, MapPin, AlertTriangle } from 'lucide-react';
...
const Events = () => {
  ...
  const handleDelete = (id: string, title: string) => {
    setConfirmModal({
      isOpen: true,
      title: 'Delete Event?',
      message: `Are you sure you want to delete "${title}"? This action is irreversible and will remove all associated ticket data.`,
  ...
  return (
    <div className="dashboard-content" style={{ animation: 'fadeInUp 0.6s ease forwards' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '4px' }}>Event Management</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>Manage events published to the TicketLiv mobile app.</p>
        </div>
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button className="icon-btn" style={{ width: 'auto', padding: '0 16px', borderRadius: '8px', gap: '8px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.01)' }}>
              ...
              <th style={{ padding: '16px 20px', fontWeight: 600, fontSize: '13px', textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'right' }}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {errorStatus ? (
